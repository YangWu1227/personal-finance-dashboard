...
add_category_modal = dbc.Modal(
    [
        ...
        dbc.ModalBody(
            dcc.Input(
                id="new_category_input", type="text", placeholder="New Category Name"
            )
        ),
        dbc.ModalFooter(
            dbc.Button(
                "Add", id="add_new_category_button", className="ms-auto", n_clicks=0
            )
        ),
    ],
...
def layout() -> dbc.Container:
    ...
    updated_dropdown_options = [
        {"label": cat, "value": cat} for cat in updated_categories
    ]
    updated_dropdown_options_with_add_new = updated_dropdown_options + [
...
def prepare_trend_data(
    spending_data: pd.DataFrame, selected_category: str, freq: str
) -> pd.DataFrame:
    """
    ...
    full_range = pd.date_range(
        start=resampled_data["timestamp"].min(),
        end=resampled_data["timestamp"].max(),
        freq=freq,
    )
...
def register_callbacks(app: dash.Dash) -> None:
    ...
    def update_category_dropdown(
        n_clicks: Union[int, None], new_category: str
    ) -> Tuple[
        List[Dict[str, str]], List[Dict[str, str]], List[Dict[str, str]], str, dbc.Alert
    ]:
        """
        ...
        if not new_category.isalnum():
            return (
                ...
                dash.no_update,
                "",
                dbc.Alert("Invalid category name.", color="warning"),
            )
        ...
        updated_options_with_add_new = updated_options + [
            {"label": "Add new", "value": "ADD_NEW"}
        ]

        return (
            updated_options_with_add_new,
            updated_options,
            updated_options,
            "",
            dbc.Alert(
                f'Category "{new_category}" added successfully!', color="success"
            ),
        )
            ...
            if amount is None or category is None:
                ...
                try:
                    ...
                    c.execute(
                        ...
                        (amount, category, spending_datetime),
                    )
                    conn.commit()
                except sqlite3.Error as e:
                    return dbc.Alert(
                        f"Database error: {e}", color="danger", duration=3000
                    )
                finally:
                    conn.close()
                return dbc.Alert(
                    f"Amount: {amount}, Category: {category} added successfully!",
                    color="success",
                    duration=3000,
                )
        return dbc.Alert(
            "Enter amount and select a category", color="warning", duration=3000
        )
    ...
    @app.callback(
        ...
        [
            Input("url", "pathname")
        ],  # Assuming you have a dcc.Location component with id='url'
    )
    def load_spending_data(pathname: str) -> Union[Dict[str, str], None]:
        ...
        """
        if (
            pathname == "/spending-tracker"
        ):  # Update the path as per your app's URL structure
            spending_data = read_spending_data(db_path)
            return spending_data.to_dict(
                "records"
            )  # Convert DataFrame to a dictionary for storage
        return dash.no_update
