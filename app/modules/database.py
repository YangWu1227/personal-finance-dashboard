...
def main() -> int:
    ...
    with sqlite3.connect(os.path.join(db_dir, "spending.db")) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS spending (id INTEGER PRIMARY KEY, amount INTEGER, category TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY, category_name TEXT)"
        )
    conn.close()
